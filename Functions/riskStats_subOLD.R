...
#      bms = cbind(sp2, hfrx$HFRXEH) 
    ...
    cbd_Sector = contributionBreakdown(id, "Sector", start = start(sub), end = end(sub))
    cbd_MktCap = contributionBreakdown(id, "Mkt_Cap", start = start(sub), end = end(sub))
    sub.ewma_volcontrib_Sector = ewmaVolatilityContribution(cbd_Sector)
    sub.ewma_volcontrib_MktCap = ewmaVolatilityContribution(cbd_MktCap)
