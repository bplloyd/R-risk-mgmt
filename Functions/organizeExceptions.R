...
#   sub.exceptions = subs.exceptions
  ...
  lse = rbind(pof.exceptions[which(row.names(pof.exceptions)=="HLSIX"),], uft.exceptions[which(row.names(uft.exceptions)=="LSE"),], sub.exceptions$LSE)
  lsd = rbind(pof.exceptions[which(row.names(pof.exceptions)=="HFINX"),], uft.exceptions[which(row.names(uft.exceptions)=="LSD"),], sub.exceptions$LSD)
  ed = rbind(uft.exceptions[which(row.names(uft.exceptions)=="ED"),], sub.exceptions$ED)
  mn = rbind(uft.exceptions[which(row.names(uft.exceptions)=="MN"),], sub.exceptions$MN)
  mf = rbind(pof.exceptions[which(row.names(pof.exceptions)=="HMFIX"),], uft.exceptions[which(row.names(uft.exceptions)=="MF"),], sub.exceptions$MF)
