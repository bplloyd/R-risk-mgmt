...
(
  ...
  WHERE 
    v.SubAdvised_UID = ", id, "
    AND ld.Cur BETWEEN '", startDate, "' AND '", endDate, "'
),
Returns AS
  (
  select
  ...
  FULL OUTER JOIN Weights AS w ON (ld.Lag = w.Date_Weight AND r.Security_UID = w.Security_UID AND r.Long_Short = w.Long_Short)
WHERE
...
path = "G:\\PORTFOLIO MANGEMENT\\Bryan Lloyd\\2016 Projects\\Data\\SP500 RData\\SP_"
dates = paste0("201605",c("12", "13", "16", "17", "18"))
data =lapply(dates, FUN = function(d)return(readRDS(file = paste0(path, d, ".rds"))))
names(data) = dates
