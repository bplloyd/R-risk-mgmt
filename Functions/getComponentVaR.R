getComponentVaR = function(subs.o, subs.weights, p = 0.98, method = "modified", n = 126, reportDate = NULL, na.rm = T)
{
    ...
    for (i in 1:length(subs.o)){
        ...
        if((sum(rowSums(is.na(assets))) > 0) & (!na.rm))
          ...
          result[[i]] = VaR(R=na.omit(assets), weights = weights, p = p, method = method, portfolio_method = "component")
    }
