marketRiskDecomposition_old2 = function(R.cov, bm.name, p = NULL, annualized = F)
{
  ...
  if(is.xts(R.cov))
  {
    ...
    R.beta = xts(t(apply(R.cov[,paste0(names(R.vol), "_", bm.name)], 
                         1, 
                         function(r)
  ...
  else
  {
    ...
    R.beta = sapply(row.names(R.cov)[-which(row.names(R.cov)==bm.name)], function(n)return(R.cov[n, bm.name]/R.cov[bm.name, bm.name]))
